...
See the [ors setup instructions](software.qmd#ors) for more details.
:::
...
# openrouteservice::ors_api_key("") # provided to you
# write('ORS_USHIFT="ushift-ors"', file = file.path(Sys.getenv("HOME"), ".Renviron"), append = TRUE) # run this only once!
```

## Buffer

Represent a buffer of 500 m and 2000 m from IST[^1].

[^1]: Here I selected only the first variable because now we also have the coordinates information (unnecessary for this procedure)
...
```{r importexportbuffer1}
#| include: false
#| message: false
# st_write(ISOCist, "original/ISOCist.gpkg", delete_dsn = TRUE)
# piggyback::pb_upload("original/ISOCist.gpkg")
ISOCist = st_read("https://github.com/U-Shift/MQAT/releases/download/2025/ISOCist.gpkg", quiet = TRUE)
```
...
```{r importexportbuffer2}
#| include: false
#| message: false
# st_write(ISOCschools, "original/ISOCschools.gpkg", delete_dsn = TRUE)
# piggyback::pb_upload("original/ISOCschools.gpkg")
ISOCschools = st_read("https://github.com/U-Shift/MQAT/releases/download/2025/ISOCschools.gpkg", quiet = TRUE)
```
