...
coor_max5_geo = st_as_sf(coor_max5, coords = c("lon", "lat"), crs = 4326)
coor_geo = st_as_sf(coor, coords = c("lon", "lat"), crs = 4326)

ISOCist = ors_isochrones(
  coor_max5,
  profile = "foot-walking",
  range_type = "time", # or distance
  range = c(5, 20)*60, #in seconds
  output = "sf"
)
...
for (i in 1:nrow(coor)) {
  ...
  ors_isochrones(
    ...
    range = c(5, 20) * 60,#in seconds
    output = "sf"
  ), error=function(e){cat("Error: Openrouteservice API request failed")}
  )
  ISOCistTEST = rbind(ISOCistTEST, ISOCistTEST_i)
}

ISOCistTEST = arrange(ISOCistTEST, -value) # to make the larger polygons on top of the table so the are displayed behind.

mapview(ISOCistTEST, zcol = "value", alpha.regions = 0.5)
```
