...
For this, we will need the census data.
...
But there is this new package [`centr`](https://ryanzomorrodi.github.io/centr) that can help us.
...
library(centr)
CENTROIDSpop = BGRI |> 
  mean_center(group = "DTMN21", # Municipalities
              weight = "N_INDIVIDUOS") # Population
```
...
CENTROIDSbuild = BGRI |> 
  mean_center(group = "DTMN21", # Municipalities
              weight = "N_EDIFICIOS_CLASSICOS") # Buildings
```
...
```{r mapcentroid}
mapview(CENTROIDSgeo) +
  mapview(CENTROIDSpop, col.region = "red") +
  mapview(CENTROIDSbuild, col.region = "black")
```
...
```{r}
# read data
MUNICIPIOSgeo = st_read("geo/MUNICIPIOSgeo.gpkg", quiet = TRUE)
LISBON = MUNICIPIOSgeo |> filter(Concelho == "Lisboa") # filter only Lisbon
```
